...
export function createVibratoEffect(context: AudioContext, id?: string): AudioEffect {
  ...
  connectToneEffect(inputGain, vibrato, wetGain, dryGain, outputGain);
  ...
  return {
    ...
    cleanup(): void {
      try {
        (vibrato as { dispose?: () => void }).dispose?.();
