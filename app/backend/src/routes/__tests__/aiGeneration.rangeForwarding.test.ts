import type { Express } from 'express';
import express from 'express';
import request from 'supertest';
import type { AuthRequest } from '../../domains/auth/infrastructure/middleware/authMiddleware';
import { authenticateToken } from '../../domains/auth/infrastructure/middleware/authMiddleware';
import { aiJobQueue } from '../../domains/ai-generation/domain/services/AiJobQueueService';
import { aiProviderFactory } from '../../domains/ai-generation/infrastructure/providers/AiProviderFactory';
import { aiSettingsService } from '../../domains/user-management/domain/services/AiSettingsService';
import { systemPressureService } from '../../shared/infrastructure/resilience/SystemPressureService';
import type { AiGenerationRequest, AiGenerationResponse } from '../../domains/ai-generation/domain/providers/AiProvider';

// Separate file (not another `it` in aiGeneration.validation.test.ts) because
// aiGenerationRateLimiter allows only 1 request per 5s per IP and is a
// module-level singleton shared by every request within a test file instance.
jest.mock('../../domains/auth/infrastructure/middleware/authMiddleware', () => ({
  authenticateToken: jest.fn(),
  optionalAuthAllowGuest: jest.fn(),
}));

jest.mock('../../domains/ai-generation/domain/services/AiJobQueueService', () => ({
  aiJobQueue: {
    createJob: jest.fn(),
    waitForSlot: jest.fn(),
    startProcessing: jest.fn(),
    completeJob: jest.fn(),
    failJob: jest.fn(),
    getQueuePosition: jest.fn(),
  },
}));

jest.mock('../../domains/ai-generation/infrastructure/providers/AiProviderFactory', () => ({
  aiProviderFactory: {
    getProvider: jest.fn(),
  },
}));

jest.mock('../../domains/user-management/domain/services/AiSettingsService', () => ({
  aiSettingsService: {
    getDecryptedKey: jest.fn(),
  },
}));

jest.mock('../../shared/infrastructure/resilience/SystemPressureService', () => ({
  systemPressureService: {
    isFeatureEnabled: jest.fn(),
    getCurrentPressure: jest.fn(),
  },
}));

// This test needs two successful POSTs, which would trip the 1-req/5s
// aiGenerationRateLimiter. It's already covered by
// aiGeneration.rangeValidation.test.ts, so mocking it here loses no coverage.
jest.mock('../../domains/ai-generation/infrastructure/middleware/rateLimit', () => ({
  aiGenerationRateLimiter: (_req: unknown, _res: unknown, next: () => void) => next(),
}));

// Import router AFTER mocking its transitive dependencies
import aiGenerationRouter from '../aiGeneration';

describe('POST /ai/generate — rangeBars reaches the provider (DEV-46)', () => {
  let app: Express;

  beforeAll(() => {
    app = express();
    app.use(express.json());
    app.use('/api/ai', aiGenerationRouter);
  });

  beforeEach(() => {
    (authenticateToken as jest.Mock).mockImplementation(
      (req: AuthRequest, _res: unknown, next: () => void) => {
        req.user = {
          id: 'user-range',
          email: 'range@example.com',
          username: 'range-user',
          userType: 'REGISTERED',
          emailVerified: true,
        } as AuthRequest['user'];
        next();
      }
    );
    (systemPressureService.isFeatureEnabled as jest.Mock).mockReturnValue(true);
    (aiSettingsService.getDecryptedKey as jest.Mock).mockResolvedValue({
      key: 'test-api-key',
      provider: 'openai',
      settings: { model: 'gpt-4' },
    });
    (aiProviderFactory.getProvider as jest.Mock).mockReturnValue({
      generate: jest.fn().mockResolvedValue({ notes: [], ops: [] }),
    });
    // AiGenerationService.generate() calls through the real job queue's
    // interface (mocked module) to get a job id before invoking the
    // provider — give it something to key off of.
    (aiJobQueue.createJob as jest.Mock).mockReturnValue({ id: 'job-range' });
    (aiJobQueue.waitForSlot as jest.Mock).mockResolvedValue(undefined);
    (aiJobQueue.getQueuePosition as jest.Mock).mockReturnValue(0);
  });

  it('forwards the requested range to aiGenerationService.generate', async () => {
    const generateSpy = jest.fn<Promise<AiGenerationResponse>, [string, AiGenerationRequest, AbortSignal?]>()
      .mockResolvedValue({ notes: [], ops: [] });
    (aiProviderFactory.getProvider as jest.Mock).mockReturnValue({ generate: generateSpy });

    const res = await request(app)
      .post('/api/ai/generate')
      .send({ prompt: 'A bassline', rangeBars: 8 });

    expect(res.status).toBe(200);
    const forwarded = generateSpy.mock.calls[0]?.[1];
    expect(forwarded?.rangeBars).toBe(8);
  });

  it('omits rangeBars entirely when the client does not send it', async () => {
    const generateSpy = jest.fn<Promise<AiGenerationResponse>, [string, AiGenerationRequest, AbortSignal?]>()
      .mockResolvedValue({ notes: [], ops: [] });
    (aiProviderFactory.getProvider as jest.Mock).mockReturnValue({ generate: generateSpy });

    const res = await request(app).post('/api/ai/generate').send({ prompt: 'A bassline' });

    expect(res.status).toBe(200);
    const forwarded = generateSpy.mock.calls[0]?.[1] ?? {};
    expect('rangeBars' in forwarded).toBe(false);
  });
});
