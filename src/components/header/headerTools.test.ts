...
describe('HEADER_TOOLS', () => {
  ...
  test('the loop layer gets the copy button, the loop picker, the vibes and the key menu', () => {
    expect(ids(headerToolsOn('loop'))).toEqual(['vibes', 'loop-copy', 'loop-selector', 'scale']);
  });
  ...
  test('the song layer gets the project name, follow toggle and export', () => {
    expect(ids(headerToolsOn('song'))).toEqual(['project-name', 'follow-playhead', 'export']);
  });
