...
describe('planPadArm', () => {
  test('pad mode arms every chord and holds for that chord alone', () => {
    const arm = planPadArm(snapshot(), { chordIndex: 1 });
  ...
  test('drone mode arms only at the top of a loop pass, and holds the whole pass', () => {
    ...
    expect(planPadArm(drone, { chordIndex: 1 })).toBeNull();
    const arm = planPadArm(drone, { chordIndex: 0 });
  ...
  test('a drone with no intervals arms nothing', () => {
    expect(planPadArm(snapshot({ mode: 'drone', intervals: [] }), { chordIndex: 0 })).toBeNull();
  });

  test('a chord index outside the progression arms nothing', () => {
    expect(planPadArm(snapshot(), { chordIndex: 9 })).toBeNull();
  });

  test('is a plain function of its inputs: two calls with the same snapshot agree', () => {
    expect(planPadArm(snapshot(), { chordIndex: 0 })).toEqual(
      planPadArm(snapshot(), { chordIndex: 0 }),
    );
  });
});
