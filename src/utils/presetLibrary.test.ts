...
import { INITIAL_EFFECTS, TRACK_SYNTH_DEFAULTS } from '@/store/initialState';
import { useAppStore } from '@/store/store';

const custom: SynthPreset = {
  id: 'custom-1',
  name: 'My Patch',
  category: 'User',
  engine: 'subtractive',
...
describe('custom preset store actions', () => {
  ...
  test('saveCustomPreset stores the complete patch and marks it non-factory', () => {
    const saved = useAppStore.getState().saveCustomPreset('My Patch', TRACK_SYNTH_DEFAULTS.bass, 'Lead');
    expect(saved.name).toBe('My Patch');
    expect(saved.engine).toBe('subtractive');
    expect(saved.patch).toEqual(TRACK_SYNTH_DEFAULTS.bass.patch);
    expect(saved.isFactory).toBe(false);
    expect(useAppStore.getState().customSynthPresets[0].id).toBe(saved.id);
  });

  test('a saved preset is a copy, and carries no pointer back to the patch it came from', () => {
    // `sourcePresetId` is provenance on a PATCH, not on a preset: the saved
    // entry IS the source now, and a copied id would make every derived patch
    // claim to be the factory one it was tweaked from.
    const saved = useAppStore.getState().saveCustomPreset('Derived', TRACK_SYNTH_DEFAULTS.bass);
    expect(saved).not.toHaveProperty('sourcePresetId');
    expect(saved.patch).not.toBe(TRACK_SYNTH_DEFAULTS.bass.patch);
  });

  test('deleteCustomPreset removes the preset and returns the new list', () => {
    const saved = useAppStore.getState().saveCustomPreset('My Patch', SUBTRACTIVE_INIT);
...
describe('ArpMode and ArpRate', () => {
  test('have exactly one definition, re-exported by the audio modules', async () => {
