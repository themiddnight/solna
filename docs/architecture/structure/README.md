...
flowchart TB
  ...
  subgraph Store["store/ — slices + second store (audioRecovery)"]
    ...
    Snap["playbackPlanSnapshots · mixdownSlice"]
...
| S4 | A key/scale change transposes Lead but not FX. | `musicContextSlice.ts` | **Fixed.** `keyChangePatch` moves every `MELODY_TRACKS` row; vibes reuse it. |
...
| U8 | Deleting a loop had no confirm and no undo. | `SortableLoopCard.tsx`, `ArrangeView.tsx` | **Fixed (Undo, no confirm).** `LoopUndoToast` + `undoLoopDelete` (`restoreLoop`, then re-activation through the same seam when the loop was active, so Undo never stops the transport). A project install dismisses a pending Undo. A confirm dialog is deferred by choice. |
