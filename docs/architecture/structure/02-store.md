...
|---|---|---|---|
| localStorage `musibox_project_state_v1` | `metronomeActive`, `selectedVibeId`, `focusTrack`, `customSynthPresets`, `customChordProgressions`, `customBeatPresets`, `activeLoopId`, `drumPadVelocities` (`partializeAppState`, `store.ts`) | zustand `persist` → `createDedupedJsonStorage` (`persistStorage.ts`) → coalesced storage | `merge` → `sanitizePersistedState` (`store.ts:193-248`, `:337-341`) |
