...
| 4 | `Workspace` | `src/App.tsx:92` | Mounts every coordinator hook and the whole visible shell. |
...
`Workspace` render order (`src/App.tsx:154-213`):

1. `Header` — `src/components/Header.tsx:591`
2. `InstantVibesBar` — only when not on the song layer (`src/App.tsx:171`) — **conditionally mounted, not gated**
3. `<main>` → `LoopPage` (gated) + `SongPage` (gated) (`src/App.tsx:182-189`)
4. `BottomInputDock` — `src/components/ui/BottomInputDock.tsx:306`
5. `UpdateBanner` — `src/components/ui/UpdateBanner.tsx:24`
6. `TransportBar` — `src/components/TransportBar.tsx:229`
7. `IncidentDialog` (also the audio-recovery modal) — `src/components/ui/IncidentDialog.tsx:154`
8. `MidiSettingsModal` — `src/components/ui/MidiSettingsModal.tsx:255`
9. `ProjectNotice` — `src/components/project/ProjectNotice.tsx:24`
...
flowchart TB
  ...
  Gate -- booted --> WS["Workspace<br/>(coordinator hooks)"]
  subgraph shell["Workspace children"]
    direction TB
    H["Header"]
    V["InstantVibesBar<br/>(loop layer only)"]
    M["main"]
    D["ui/BottomInputDock"]
    U["ui/UpdateBanner"]
    T["TransportBar"]
    I["ui/IncidentDialog"]
    MI["ui/MidiSettingsModal"]
    PN["project/ProjectNotice"]
  end
  WS --> shell
  M --> LP["loop/LoopPage (hidden unless loop layer)"]
  M --> SP["song/SongPage (hidden unless song layer)"]
```
...
flowchart LR
  subgraph Header["Header.tsx"]
    PM["project/ProjectMenu<br/>(Wordmark dropdown)"]
    LS["LayerSwitcher (Loop/Song)"]
    LCB["loop/LoopCopyButton<br/>(loop layer)"]
    LSel["loop/LoopSelector<br/>(loop layer)"]
    PNL["ProjectNameLabel<br/>(song layer)"]
    FPT["FollowPlayheadToggle<br/>(song layer)"]
    EXP["ExportButton — mixdown WAV<br/>(song layer)"]
    SM["ScaleMenu / ScaleSelects<br/>(loop layer)"]
    TB["TabButton × tabsForLayer"]
    TH["theme IconButton"]
  end
  ...
  subgraph Transport["TransportBar.tsx"]
    ...
    VU["ui/VuMeter"]
    MFd["MasterFader → ui/VolumeFader"]
  end
...
flowchart TB
  ...
  SV --> MIX["loop/SoundMixer → ui/ChannelStrip, ui/SourceMeter, ui/PowerToggle"]
...
flowchart TB
  ...
  L -. hooks .-> LH["useLeadPlayback · useLeadStepPublisher · useLeadGridModel/Transport/Commands"]
  ...
  A -. hooks .-> CH["chord/useChordView → chord/useChordPlayback"]
  ...
  SG -. hook .-> USP["useSequencerPlayback"]
```
...
| 13 | Loop / Song layer switch | Header | `Header.tsx:508` |
| 14 | View tabs (Sound, Pattern / Arrange, Master FX) | Header | `Header.tsx:57,655-665`, `viewMeta.ts` |
...
| 17 | Project name edit (Enter/Escape/blur) | Header (song layer) | `Header.tsx:173-232` |
| 18 | Follow playhead toggle | Header (song layer) | `Header.tsx:233` |
| 19 | Export mixdown (WAV) + progress + cancel | Header (song layer) dropdown | `Header.tsx:295,354-432` |
| 20 | Master key & scale (inline ≥xl, dropdown below) | Header (loop layer) | `Header.tsx:98,542` |
| 21 | Light/dark theme toggle (persisted `solna_theme`) | Header, right | `Header.tsx:434-506,669-682` |
| 22 | Instant Vibes chips (8) — load vibe | Vibes bar (loop layer) | `InstantVibesBar.tsx:200`, `vibeActions.ts` |
| 23 | Vibe dice reroll | Vibes bar chip | `InstantVibesBar.tsx:161-162,216` |
| 24 | Vibe load/reroll toast | Vibes bar | `InstantVibesBar.tsx:176` |
| 25 | Vibe action prefetch (lazy chunk after 1.2 s / hover) | Vibes bar | `InstantVibesBar.tsx:53-78` |
| 26 | Play / soft stop / hard stop (loop solo vs song playAll) | Transport bar | `TransportBar.tsx:150,300-306`, `ui/PlayerTransport.tsx` |
| 27 | Play-target label + Song-mode badge | Transport bar | `TransportBar.tsx:22,172-186` |
| 28 | BPM field with −/+ | Transport bar | `TransportBar.tsx:49` |
| 29 | Time signature (meter) select | Transport bar | `TransportBar.tsx:90`, `meterSelect.ts` |
| 30 | Now/next chord + beat dots readout (≥xl only) | Transport bar | `PlayheadReadout.tsx`, `TransportBar.tsx:352` |
| 31 | Metronome toggle | Transport bar | `TransportBar.tsx:112` |
| 32 | MIDI activity indicator (opens MIDI settings) | Transport bar | `ui/MidiIndicator.tsx` |
| 33 | Incident / audio-recovery warning chip | Transport bar | `ui/IncidentDialog.tsx:137` |
| 34 | Master VU meter | Transport bar | `ui/VuMeter.tsx` |
| 35 | Master volume fader | Transport bar | `TransportBar.tsx:207` |
...
| 39 | Project notice (parse/export/autosave failures) | Root banner | `project/ProjectNotice.tsx` |
...
| 68 | Mixer: 6 channel strips (level, mute, meter) in 3 groups | Sound, bottom | `loop/SoundMixer.tsx:25-47,234` |
...
| 80 | Progression editor: add chord, quick-add palette (7ths), reorder (drag, ←/→), delete, root/quality/bars selects, hold-to-preview | Accompaniment | `loop/chord/ProgressionCard.tsx`, `chord/SortableChordCard.tsx` |
...
| # | Feature | Where | File |
|---|---|---|---|
| 90 | Arrange: add loop; per card select/cue, rename, edit (deep link to Sound), move up/down, duplicate, copy-parts-into, delete (with a timed Undo toast, `song/LoopUndoToast.tsx`), drag reorder, audition "play only", repeat count, per-loop channel mixer, chord strip, progress rail, follow-playhead scroll | Arrange | `song/ArrangeView.tsx:229-431`, `song/SortableLoopCard.tsx:225-798` |
| 91 | Copy parts between loops (quick chips, per-group checkboxes, key/bar notices) | Arrange → dialog | `song/LoopCopyDialog.tsx:327` |
| 92 | Change the key of several selected loops at once — Set (one key for all) or Transpose (each loop's own root shifted, scale type kept), old-to-new preview per loop, with a timed Undo toast sharing the loop-delete toast's container | Arrange → dialog | `song/KeyChangeDialog.tsx`, `song/useKeyChangeDialog.ts`, `song/useLoopKeyChangeUndo.ts` |
...
|---|---|---|---|
| `Header` (`Header.tsx`) | `activeTab`, `scaleRoot`, `scaleType`, `projectName`, `followPlayhead`, `mixdownProgress`, `selectMixdownBusy` | `setActiveTab`, `setScaleRoot`, `setScaleType`, `setProjectName`, `toggleFollowPlayhead`, `exportMixdown`, `cancelMixdown`, `setMixdownProgress`, `setProjectNotice` | `store/mixdownSlice` selector; guarded `localStorage` for theme; `document.documentElement[data-theme]` |
| `ProjectMenu` | `projectName`, `projectSource`, `drive*`, `selectMixdownBusy` | `newProject`, `saveProject`, `saveProjectAsLocal`, `openProjectFile`, `exportProjectFile`, `openFromDrive`, `saveAsToDrive`, `listDriveProjects`, `connectDrive`, `disconnectDrive`, `setProjectNotice` | `store/driveClient`, `store/projectFile`, `store/incidentReporter` (`reportManualIncident`), lazy `@/diagnostics/DiagnosticPanel` |
| `InstantVibesBar` | `selectedVibeId`, `bpm`, `scaleRoot`, `scaleType` | via lazy `vibeActions.ts` → `applyVibeToStore(resolveVibe…)` | `data/vibes`, `store/vibeVariation`, `Math.random` |
...
| `SoundMixer` | `isAnyPlayerActive`, per-channel volume/muted | `setSynthVolume`…`setBeatLevel`, `toggle*Muted` (`SoundMixer.tsx:25-47`) | `ui/SourceMeter` → `audioEngine` analysers |
...
| `ChordView` + `useChordView` | `chords`, `scaleRoot/Type`, `chordOctave`, `chordRhythm*`, `bassPattern*`, `customChord*`, `customBass*`, `meterId`, `bpm`, `playheadChord*` (+ `usePlayheadBeat`), `spellingKey`, `customChordProgressions` | `setChords`, `saveCustomChordProgression`, `deleteCustomChordProgression` | mounts `useChordPlayback` (clock subscription, note-on/off via `audio/playback/playbackEngine`, planners `plan/chordPlan`, `plan/padPlan`); `audio/playback/chordPlayback` for previews |
...
| `LeadMelodyGrid` (×2) | `leadMelodySteps`/`fxMelodySteps` etc. via `MELODY_TRACKS`, `focusTrack`, `recordingTrack`, `meterId`, `bpm`, `scaleRoot/Type`, `chords` | lead-slice actions via `store/leadSlice`, `setRecordingTrack` | `useLeadPlayback` (clock → `plan/melodyPlan`), `useLeadStepPublisher` (publishes to `playbackStep.ts`), `audio/leadMelody`, `audio/leadLiveRecord` |
| `SequencerView` + `SequencerGrid` | `beatPattern`, `beatMix`, `meterId`, `sequencerPlayer` | `replaceBeatPattern`, `toggleBeatStep`, `toggleBeatVoiceMuted`, `setBeatVoiceLevel` | `useSequencerPlayback` (clock subscription, `plan/beatPlan` (`planBeatStep`), `drumPlayback`), `useSegmentGatedStep` |
...
| `KeyChangeDialog` + `useKeyChangeDialog` | — | `applyLoopKeyChange` (via `useLoopKeyChangeUndo`) | props from `ArrangeView`: `loops`, `activeLoopId` (old-to-new preview); `useLoopKeyChangeUndo` shares the loop-delete Undo toast's timer (`song/loopUndo.ts`, `LOOP_UNDO_MS`) and container |
...
beat once per beat through the local `playheadBeat` pub/sub, `components/playheadBeat.ts`), `useSequencerPlayback.ts`, `loop/chord/useChordPlayback.ts`
(`useChordClock`, `:556`), `loop/lead/useLeadPlayback.ts`, `loop/lead/useLeadStepPublisher.ts`,
`song/ArrangeView.tsx:142`. Step fan-out to views goes through the module singleton
...
|---|---|---|
| Layer | `src/App.tsx:183-188` | `isSongLayer(activeTab)` → `block`/`hidden` on `LoopPage` / `SongPage` wrappers |
...
| Vibes bar | `src/App.tsx:171` | **Unmounted** on song layer (conditional render, not gating) |
| Sound synth vs Beat editor | `loop/SoundView.tsx:225-236` | **Unmounted/remounted** on focus change between melodic and drum |
| Song-only header controls | `Header.tsx:173-177,233-240,354-362` | return `null` off the song layer |
...
1. **Mixdown export is not in the project menu.** feature-overview row 10 says "Project menu";
   it is a song-layer Header dropdown (`Header.tsx:354-432`, comment at `Header.tsx:635-638`).
   `ProjectMenu` has no export row (`ProjectMenu.tsx:86-116`).
...
8. **"Components are dumb views" is not what `src/components/` holds.** It hosts all four live
   ...
   **Fixed on `fix/structure-audit-bugs`:** both docs now name the controllers and where they live; moving them is deferred.
...
| 710 | `src/components/loop/chord/useChordPlayback.ts` |
...
- `useChordView.ts` is seven hooks (`:56-548`) that together are a controller for the whole
  Accompaniment segment, and it is also the mount point for `useChordPlayback`
  (`useChordView.ts:79`) — chord/bass/pad playback exists only because a view hook of the
  Pattern segment is mounted.

**Logic that looks misplaced**

- Playback is owned by grid components: drums play because `SequencerGrid` calls
  `useSequencerPlayback()` (`loop/sequencer/SequencerGrid.tsx:50`); lead/FX because
  `LeadMelodyGrid` calls `useLeadPlayback` (`LeadMelodyGrid.tsx:251`); chords via
  `useChordView.ts:79`. Correct only because every segment stays mounted; unmounting any
  segment would silence its lane. (A structural coupling, not a current bug.)
...
- `vibeActions.ts` is store orchestration (`applyVibeToStore`, `useAppStore.getState()`,
  `Math.random`) living in `src/components/` (`vibeActions.ts:13-35`).
...
- `useArpPlayback` — a React hook — lives in `src/audio/playback/arpPlayback.ts:160` (imports
  `react` at `:1`), the mirror image of controllers living in components. (Outside this doc's
  scope; noted for the audio-layer doc.)
...
- At least six hand-rolled toast timers with no shared primitive (on `fix/structure-audit-bugs`,
  ...
  are deferred):
  `InstantVibesBar.tsx:28-43`, `loop/ChordPresetLibrary.tsx:87`,
...
- Components root has 13 non-component modules (`playbackStep.ts`, `playerStop.ts`,
  ...
  `fxDescriptors.ts`, `vibeActions.ts`, `visualizerDraw.ts`, `viewMeta.ts`, …).
- Playback hooks are scattered: `useSequencerPlayback.ts` at root, `useLeadPlayback.ts` in
  `loop/lead/`, `useChordPlayback.ts` in `loop/chord/`.
...
- ~~Deleting a loop has no confirmation and there is no undo.~~ **Fixed on `fix/structure-audit-bugs`:** deleting a loop shows a
  timed Undo toast (`song/LoopUndoToast.tsx`, `restoreLoop`); a confirm dialog was deliberately not
  added. A vibe click rewrites the loop and the code notes "there is no
  undo" (`InstantVibesBar.tsx:244-245`). Only project-replacing actions and preset deletes use
  `ConfirmDialog`.
