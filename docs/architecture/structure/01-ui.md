...
2. The layout shell — `DesktopShell` or `MobileShell` (`src/components/shell/`), chosen by
   ...
   - `Header`
   - `InstantVibesBar` — only when not on the song layer — **conditionally mounted, not gated**
   ...
   - `InstantVibesBar` — only when not on the song layer — same rule as desktop
   - `<main>` → `shell/LayerPages`
   - `BottomInputDock`
   - `UpdateBanner`
   - `TransportBar` — without the bottom inset
...
flowchart LR
  subgraph Header["Header.tsx (desktop frame only)"]
    ...
    LS["LayerSwitcher (Loop/Song, structural)"]
...
| Vibes bar | the shells (`shell/DesktopShell.tsx`, `shell/MobileShell.tsx`) | **Unmounted** on song layer (conditional render, not gating) |
| Sound synth vs Beat editor | `loop/SoundView.tsx:225-236` | **Unmounted/remounted** on focus change between melodic and drum |
| `HEADER_TOOLS` `layers` (`header/headerTools.ts`) | `headerToolsFor(layer, group)` | not rendered off the song layer |
