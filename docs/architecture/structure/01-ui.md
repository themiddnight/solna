...
"Where" = the surface a user sees it on. 92 rows.
...
| `SequencerView` + `SequencerGrid` | `beatPattern`, `beatMix`, `meterId`, `sequencerPlayer` | `replaceBeatPattern`, `toggleBeatStep`, `toggleBeatVoiceMuted`, `setBeatVoiceLevel` | `useSequencerPlayback` (clock subscription, `audio/beatSteps`, `drumPlayback`), `useSegmentGatedStep` |
...
| `KeyChangeDialog` + `useKeyChangeDialog` | `loops` (old-to-new preview) | `applyLoopKeyChange` (via `useLoopKeyChangeUndo`) | `useLoopKeyChangeUndo` shares the loop-delete Undo toast's timer (`song/loopUndo.ts`, `LOOP_UNDO_MS`) and container |
