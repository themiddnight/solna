...
`useRouteSync`, `usePlayheadSync`, `useSongModeSync`, `useSoloNavClear`,
...
| 23 | Vibe dice reroll (one per row) | Vibe picker row | `vibes/VibePickerModal.tsx`, `vibes/useVibePicker.ts`, `store/vibePreview.ts` (`rerollPreview`) |
...
| 48 | Drum pads (10 pads, no level control: each strikes at `BEAT_PREVIEW_VELOCITY`, the level is the Beat-mix fader — ADR-0047) | Dock → Drums | `ui/DrumPadGrid.tsx`, `components/useInputDeck.ts` |
...
| `BottomInputDock` (via `useBottomInputDock`) | `isInputPanelOpen`, `inputTargetOf` (`focusTrack`, `inputTargetPin`, `recordingTrack`) (+ props from `useInputDeck`) | `setIsInputPanelOpen`, `setFocusTrack`, `setInputTargetPin`, `setKeyboardMode` (prop) | — |
| `useInputDeck` (in `Workspace`) | `keyboardMode`, `scaleRoot/Type`, `bpm`, `inputTargetOf`, the target's synth + arp via `SYNTH_PARAM_FIELD`/`SYNTH_ARP_FIELD` | `setKeyboardMode`, `setDrumPadVelocity` | `audio/playback/{synthPlayback,arpPlayback,heldNotes,noteInputBus,drumPlayback}`; `window` key listeners; local state for octave, active notes, pad-velocity drag preview |
