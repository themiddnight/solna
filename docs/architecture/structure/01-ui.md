...
| `useVibePicker` (`vibes/`) | `isAnyPlayerActive` only | via the lazy `import('@/store/vibePreview')` → the preview commands (`beginVibePreview` … `cancelVibePreview`); `showFeedback` on Use | `data/vibes` |
