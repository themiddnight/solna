...
- **Playable event** — a fully resolved, timestamped instruction: pitch already resolved to a frequency or canonical note name, timing already resolved to an `AudioContext` time, voice ownership (`VoiceOwner`) already assigned. The sole output type of a playback planner (see below). A planner may still emit a canonical note name — a controller between the planner and the engine resolves it to Hz (`noteFrequency`, at the boundary `src/architecture/frequencyBoundary.test.ts` pins) before the one call the audio engine itself accepts (`AudioEngine.triggerSynthNoteOn`, `frequency: number`, DEV-399) — so "ready for the audio engine" describes the value only after that resolution, not a planner's own output. No playable event may name a scale, a key, a chord quality, or a Tonal.js type.
