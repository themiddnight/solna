...
like `components/settings/useThemeChoice.ts`'s theme functions take an injectable storage param and read it *inside* a `try`,
...
- **R244** — Storage access is guarded: `localStorage` can throw; `store.ts` falls back to in-memory
  ...
  default-parameter expression (`settings/useThemeChoice.ts` theme functions).
