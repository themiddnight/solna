...
The live playback controllers — `useChordPlayback`, `useLeadPlayback`, `useSequencerPlayback`,
`useInputDeck`, `usePlayheadSync` — are mounted inside the grids: **a lane sounds because its grid
is mounted**, which is one more reason every view stays mounted. (Where controllers live and what
they may import is [ADR-0002](0002-four-layer-import-architecture.md).)
...
- **R040** — Controllers are mounted inside the grids: a lane sounds because its grid is mounted.
