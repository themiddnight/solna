...
2. `src/store/` — never imports `components/`. One Zustand store composed from slices (`transport`, `musicContext`, `synth`, `chords`, `bass`, `sequencer`, `effects`, `ui`, `presets`), with `persist` (key `musibox_project_state_v1`, **version 3**, `partialize` + `migrate` in `store.ts`, legacy-key adoption in `migrate.ts`) and `subscribeWithSelector`.
3. `src/components/` — dumb views; must not import `audio/engine`. Only `AudioVisualizer.tsx` and `TransportBar.tsx` (read-only analyser consumers) and test files are exempt.
